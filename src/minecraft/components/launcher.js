...
// Получение текущего пути файла (замена __dirname в ESM)
...
class MCLCore extends EventEmitter {
  async launch (options) {
    try {
      ...
      if (!this.options.version || !this.options.version.number) {
        throw new Error('Version configuration is missing or invalid');
      }
      ...
      const java = await this.handler.checkJava(this.options.overrides.javaPath || 'java');
      if (!java.run) {
  ...
  printVersion() {
    // Адаптация для ES модулей - используем import.meta.url
    const packagePath = path.join(__dirname, '..', 'package.json');
    if (fs.existsSync(packagePath)) {
      try {
        const packageData = JSON.parse(fs.readFileSync(packagePath, 'utf-8'));
        this.emit('debug', `[MCLC]: MCLC version ${packageData.version}`);
      } catch (e) {
        this.emit('debug', '[MCLC]: Error reading package.json: ' + e);
      }
    } else { 
  ...
  startMinecraft(launchArguments) {
    // Для Windows используем javaw.exe если debug выключен
    const javaExecutable = process.platform === 'win32' && !this.options.overrides.showConsole 
      ? (this.options.overrides.javaPath ? this.options.overrides.javaPath.replace('java.exe', 'javaw.exe') : 'javaw')
      : (this.options.overrides.javaPath ? this.options.overrides.javaPath : 'java');

    const minecraft = child_process.spawn(
      javaExecutable,
      launchArguments,
      { 
